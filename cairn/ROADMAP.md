...
_Last hygiene check: 2026-08-08 (M09 merged and archived; M04 pruned under terminal-row retention; M10 is the only workable milestone and its dependency is now met)_
...
| M10 | Command-contract coverage gate — completeness observed, not inferred | in-progress | M09 | high | milestones/M10-coverage-gate-run-scope.md |
...
- A test file gated by a top-level `skip_on_cran()` / `skip_if_not_installed()` executes no `test_that()`, so the coverage gate reports it as never run and a declared-full run (CI/CRAN) FAILs — record the files testthat sources, not only those with an executed test — added 2026-08-08 — M10 review 2, D20 (reproduced)
