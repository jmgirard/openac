...
- [x] AC5 (BC5,BC7,BC10): run modes and deletions. Full `devtools::test()`: FAIL
      ...
      `enforce_pass`, 0 contract skips, 0 failures. NO INSTALL-SITE
      RECORDING: the requirement is behavioral — `local_fake_tools()` and
      `local_fake_downloads()` leave the recorded-file set unchanged — while
      `grep -rnE 'harness_runs|harness_test_files|harness_files|registry\$(runs|files)'
      tests/testthat/` giving no output (exit 1) is HYGIENE only: it catches a
      reintroduction and evidences no deletion, those names never having
      existed on `main`. Ordering: no expected file sorts after the contract
...
- [x] T11: amend AC5's deletion clause — drop the deletion that never happened.
...
- 2026-08-08: amendment return: AC5 — "NO INSTALL-SITE RECORDING: the requirement is behavioral — `local_fake_tools()` and `local_fake_downloads()` leave the recorded-file set unchanged — while `grep -rnE 'harness_runs|harness_test_files|harness_files|registry\$(runs|files)' tests/testthat/` giving no output (exit 1) is HYGIENE only: it catches a reintroduction and evidences no deletion, those names never having existed on `main`." — convened by explicit user override of the second-amendment-return stop; the false claim that `test-helper-boundary.R` carried a deleted `harness_files()` assertion is gone.
- 2026-08-08: T11 — AC5 amended and un-ticked pending re-verification; no code changed, `devtools::test()` `[ FAIL 0 | WARN 0 | SKIP 2 | PASS 541 ]`. AC1-AC4 and AC6 keep the second-pass evidence. Status → review.
