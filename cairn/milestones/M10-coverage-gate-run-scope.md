...
- [ ] AC1 (BC1,BC2,BC9): completeness is OBSERVED, not inferred. `ran` is
      ...
      `TESTTHAT_PARALLEL`.
- [ ] AC2 (BC6): the skip/fail/enforce decision is a pure function of
...
- [ ] AC3 (BC3,BC4,BC8): the gate fails CLOSED. The contract file's first test
...
- [ ] AC4 (BC5,BC7,BC10): healthy-tree run modes and the deletions. Full
...
- [ ] AC5: `Rscript -e 'devtools::test()'` clean and `Rscript -e
      'devtools::check()'` clean (0 errors, 0 warnings; the standing spelling
      NOTE justified), and `R CMD check` green on all five CI platforms in this
      milestone's PR.

### Deviations from RR02

BC1–BC10 are consolidated into AC1–AC4 rather than appended one-per-criterion;
...
| BC5 | Folded into AC4; the filtered-run skip reason now names every non-run test file across all 13, not the 7 harness files | BC1's content-free expected set widens the message; BC5's wording would otherwise read as a mismatch against prior evidence |
...
| BC10 | Dropped "pass count >= 504" and the never-shrinks tolerance; dropped the `check()` and five-platform clauses into AC5 | The floor encodes one machine's tool inventory (ffmpeg/ffprobe installed there contribute passes), contradicts the deletions BC7 mandates, and a count floor is the gate class PROFILE forbids |
...
- 2026-08-08: criteria audit ran before the criteria were committed — an [O] reader that authored none of them asked the three questions of AC1-AC5 and of the set.
