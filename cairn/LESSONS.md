...
- 2026-08-07 (M09, retiring the M06/M08 executability lesson): `Sys.which()` was MEASURED on GitHub runners rather than reasoned about, twice having been guessed wrong. Unix resolves an existing file iff `file.access(path, 1L) == 0`, extension irrelevant. Windows resolves it iff it carries any extension — `.txt` resolved at 0755 while `file.access()` said -1, so mode plays no part there — or, extensionless, iff a `<path>.exe` sibling exists. So a Windows test fixture must carry an extension, and `test-helper-boundary.R` now fails on the mistake the old lesson only warned about.
