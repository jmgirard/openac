...
- Green up `R CMD check`: fix 3 stale vignettes (`extract_opensmile`→`os_extract` etc.), document `install_opensmile_{mac,win}`/`os_check_config`, add utils/stats importFroms, `.Rbuildignore` `.claude` — added 2026-07-11 — surfaced by M01
