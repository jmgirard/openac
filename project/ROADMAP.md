...
|---|---|---|---|---|---|
| M01 | openSMILE tidy reader (`os_read`) | review | — | high | [M01](milestones/M01-opensmile-reader.md) |
...
- Green up `R CMD check`: fix 3 stale vignettes (`extract_opensmile`→`os_extract` etc.), document `install_opensmile_{mac,win}`/`os_check_config`, add utils/stats importFroms, `.Rbuildignore` `.claude`, resolve roxygen pin (project 7.3.3 vs installed 8.0.0) — added 2026-07-11 — surfaced by M01
