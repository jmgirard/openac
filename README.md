...
- **`ffm` is the sharpest.** In openac it is an alias for `ffmpeg()` and
  returns the tool’s output. In tidymedia it constructs a command object
  and runs nothing.
- **The `find_*` and `set_*` pairs read and write different
  configuration directories.** A tool location recorded with
  `openac::set_ffmpeg()` is invisible to `tidymedia::find_ffmpeg()`, and
  the reverse. Register each tool in each package you use it from.
- **`set_program()` accepts different program names** in each package.
